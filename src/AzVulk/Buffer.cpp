...
// New Buffer Struct
    ...
    void BufferData::createBuffer(
        const Device& vulkanDevice, std::vector<T>& data,
        ...
        // No need to free memory since it is freed in the cleanup() function already
        if (!keepMapped) return;
