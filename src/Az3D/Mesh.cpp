...
namespace Az3D {
    ...
    // BVH Implementation - exact copy from original Map.hpp
    void Mesh::createBVH() {
        if (indices.empty()) {
    ...
    HitInfo Mesh::closestHit(const glm::vec3& origin, const glm::vec3& direction, float maxDistance, const Transform& trform) const {
        HitInfo hit;
    ...
    HitInfo Mesh::closestHit(const glm::vec3& sphere_origin, float sphere_radius, const Transform& trform) const {
        HitInfo hit;
        if (!useBVH || nodes.empty()) {
            return hit; // No BVH available
        }
