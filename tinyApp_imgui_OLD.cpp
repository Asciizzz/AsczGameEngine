...
void tinyApp::renderAnimationEditor(tinySceneRT* activeScene, const tinyNodeRT* animNode) {
    ...
    // ===== HEADER =====
    ImGui::Text("Animation System: %s", animNode->name.c_str());
    ImGui::Separator();
    ImGui::Spacing();

    // ===== LAYER MANAGEMENT =====
    ImGui::TextColored(ImVec4(0.5f, 1.0f, 0.8f, 1.0f), "LAYERS");
    ImGui::SameLine();
    if (ImGui::Button("+ Add Layer")) {
        static int layerCounter = 0;
        std::string newLayerName = "Layer_" + std::to_string(layerCounter++);
        animComp->addLayer(newLayerName, tinyRT_ANIM3D::BlendMode::Normal);
    }
    ...
    static std::string selectedStateName;
    
    ImGui::BeginChild("LayerList", ImVec2(0, 200), true);
    
    for (const auto& [layerName, layer] : animComp->layers) {
        ImGui::PushID(layerName.c_str());
        
        bool isSelected = (selectedLayerName == layerName);
        
        // Layer header
        if (ImGui::CollapsingHeader(layerName.c_str(), ImGuiTreeNodeFlags_DefaultOpen)) {
            ImGui::Indent();
            
            // Layer controls
            ImGui::Checkbox("Enabled", const_cast<bool*>(&layer.enabled));
            
            // Blend mode
            const char* blendModes[] = { "Normal", "Additive", "Override" };
            int currentMode = static_cast<int>(layer.blendMode);
            if (ImGui::Combo("Blend Mode", &currentMode, blendModes, 3)) {
                const_cast<tinyRT_ANIM3D::AnimLayer&>(layer).blendMode = static_cast<tinyRT_ANIM3D::BlendMode>(currentMode);
            }
            
            // Weight
            ImGui::SliderFloat("Weight", const_cast<float*>(&layer.weight), 0.0f, 1.0f);
            
            // Current state
            if (!layer.currentState.empty()) {
                ImGui::TextColored(ImVec4(0.3f, 1.0f, 0.3f, 1.0f), "Current: %s", layer.currentState.c_str());
            } else {
                ImGui::TextColored(ImVec4(0.7f, 0.7f, 0.7f, 1.0f), "No active state");
            }
            
            // Add state button
            if (ImGui::Button("+ Add State to Layer")) {
                ImGui::OpenPopup("AddStatePopup");
            }
            
            // Popup for selecting animation clip
            if (ImGui::BeginPopup("AddStatePopup")) {
                ImGui::Text("Select Animation Clip:");
                ImGui::Separator();
                
                for (const auto& [clipName, clipHandle] : animComp->MAL()) {
                    if (ImGui::Selectable(clipName.c_str())) {
                        // Add state with this clip
                        tinyRT_ANIM3D::AnimState newState;
                        newState.clipHandle = clipHandle;
                        newState.timeScale = 1.0f;
                        newState.loop = true;
                        
                        // Access layer mutably
                        tinyRT_ANIM3D::AnimLayer* mutableLayer = animComp->getLayer(layerName);
                        if (mutableLayer) {
                            mutableLayer->states[clipName] = newState;
                            
                            // Set as current state if this is the first one
                            if (mutableLayer->currentState.empty()) {
                                mutableLayer->currentState = clipName;
                                // Activate the state
                                tinyRT_ANIM3D::AnimState* state = mutableLayer->getState(clipName);
            ...
            if (!layer.states.empty()) {
                ImGui::Spacing();
                ImGui::TextColored(ImVec4(0.8f, 0.8f, 0.5f, 1.0f), "States:");
                
                for (const auto& [stateName, state] : layer.states) {
                    ImGui::PushID(stateName.c_str());
                    
                    bool isActive = (layer.currentState == stateName && state.active);
                    
                    if (isActive) {
                        ImGui::PushStyleColor(ImGuiCol_Button, ImVec4(0.3f, 0.7f, 0.3f, 1.0f));
                    }
                    
                    if (ImGui::Button(stateName.c_str(), ImVec2(-1, 0))) {
                        // Transition to this state
                        selectedLayerName = layerName;
                        selectedStateName = stateName;
                        animComp->transitionToImmediate(layerName, stateName);
                    }
    ...
    // ===== STATE DETAILS =====
    if (!selectedLayerName.empty() && !selectedStateName.empty()) {
        tinyRT_ANIM3D::AnimLayer* layer = animComp->getLayer(selectedLayerName);
        if (layer) {
            tinyRT_ANIM3D::AnimState* state = layer->getState(selectedStateName);
            if (state) {
                ImGui::TextColored(ImVec4(0.5f, 0.8f, 1.0f, 1.0f), "STATE: %s (Layer: %s)", selectedStateName.c_str(), selectedLayerName.c_str());
                ImGui::Separator();
                
                // Get the clip
                tinyRT_ANIM3D::Clip* clip = animComp->get(state->clipHandle);
                if (clip) {
                    // Playback controls
                    ImGui::Text("Clip: %s", clip->name.c_str());
                    ImGui::Text("Duration: %.2f s", clip->duration);
                    
                    ImGui::Spacing();
                    
                    // State controls
                    ImGui::SliderFloat("Time Scale", &state->timeScale, 0.0f, 3.0f);
                    ImGui::Checkbox("Loop", &state->loop);
                    
                    ImGui::Spacing();
                    
                    // Current time display
                    ImGui::Text("Time: %.2f / %.2f s", state->time, clip->duration);
                    
                    // Progress bar
                    float progress = clip->duration > 0.0f ? (state->time / clip->duration) : 0.0f;
    ...
    // ===== ALL CLIPS =====
    ImGui::TextColored(ImVec4(0.8f, 0.8f, 0.5f, 1.0f), "ALL ANIMATION CLIPS");
    ImGui::Separator();
    
    ImGui::BeginChild("AllClips", ImVec2(0, 150), true);
    
    for (const auto& [clipName, clipHandle] : animComp->MAL()) {
        const tinyRT_ANIM3D::Clip* clip = animComp->get(clipHandle);
        if (clip) {
            ImGui::Text("%s - %.2fs (%zu channels)", 
                       clip->name.c_str(), 
                       clip->duration,
                       clip->channels.size());
        }
